window.onload = function () {
  ...
  async function obtenerPaises() {
    try {
      const response = await fetch(
        "http://api.geonames.org/countryInfoJSON?username=michael.giraldo"
      );
  ...
  async function obtenerCiudades(paisCode) {
    try {
      const response = await fetch(
        `http://api.geonames.org/searchJSON?country=${paisCode}&maxRows=10&username=michael.giraldo`
      );
      const data = await response.json();
  ...
  async function registro(usuarioR) {
    try {
      const response = await fetch(
        "http://localhost:3000/api/insercionUsuario",
        {
          method: "POST",
          headers: {
